...
# === FILE CHE AGGIORNA SDDM ===

# File da modificare
CONFIG_FILE="/usr/share/sddm/themes/sugar-candy/theme.conf"
COLOR_FILE="/home/lukil/.cache/wal/colors.css"
BACKGROUND_PATH="/usr/share/sddm/themes/sugar-candy/Backgrounds/"

cp "$1" "$BACKGROUND_PATH/active_background.${1##*.}"

COLOR=$(grep color2 "$COLOR_FILE" | cut -d'#' -f2 | tr -d ';')

cat > "$CONFIG_FILE" << EOF
x[General]

Background="Backgrounds/active_background.${1##*.}"
